...
function activate(context) {
    ...
    const updateClock = () => {
        ...
        const now = new Date();
        const clockString = now.toLocaleTimeString(locale === 'default' ? undefined : locale, {
            hour: '2-digit',
            minute: '2-digit',
    ...
    const configureClockCommand = vscode.commands.registerCommand('novaclock.configure', async () => {
        ...
        const choice = await vscode.window.showQuickPick([...baseOptions, ...pomodoroOptions], { placeHolder: 'Select what to change' });
        if (!choice)
            return;
        switch (choice) {
            ...
            case 'Start Pomodoro': {
                const shouldStart = await vscode.window.showInformationMessage('Ready to start working?', 'Yes, start timer', 'No, cancel');
            ...
            case 'Resume work': {
                const shouldResume = await vscode.window.showInformationMessage('Ready to get back to work?', 'Yes, start working', 'No, continue break');
            ...
            case 'Toggle break auto-start': {
                ...
                vscode.window.showInformationMessage(newAutoStartBreak
    ...
    const startBreak = (isLong) => {
        ...
        const now = new Date();
        const breakDuration = (isLong ? settings.longBreakTime : settings.shortBreakTime) * 60 * 1000;
    ...
    const getTimeLeft = () => {
        ...
        const timeLeft = Math.max(0, Math.ceil((pomodoroState.endTime - now) / 1000));
        return timeLeft;
    };
            ...
            pomodoroState.pomodoroInterval = setInterval(async () => {
                ...
                if (currentTime >= pomodoroState.endTime) {
                    ...
                    if (pomodoroState.isBreak) {
                        ...
                        const shouldStartWork = await vscode.window.showInformationMessage('Break time is over! Ready to start working?', 'Yes, start working', 'Need more time');
                        if (shouldStartWork === 'Yes, start working') {
                            ...
                            vscode.window
                                .showInformationMessage(message, shortBreakOption, longBreakOption, stopOption)
                                .then((choice) => {
                                switch (choice) {
                        ...
                        if (currentSettings.autoStartBreak) {
                            ...
                            vscode.window.showInformationMessage('Great work! Time for a break!');
                        }
                        else {
